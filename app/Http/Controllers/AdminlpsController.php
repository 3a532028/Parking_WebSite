    ...
    public function sort($fun){
        switch ($fun)
        {
            case 'ent':
                ...
                $Lp_list2=Lp::all()->whereNotIn('status','已進場')->sortBy('status');
                $Lp_list=$Lp_list1->merge($Lp_list2);
                return $Lp_list;
                break;
            case 'unent':
                $Lp_list1=collect(Lp::all()->where('status','未進場'));
                $Lp_list2=Lp::all()->whereNotIn('status','未進場')->sortBy('status');
                $Lp_list=$Lp_list1->merge($Lp_list2);
                return $Lp_list;
                break;
            case 'over':
                $Lp_list1=collect(Lp::all()->where('status','已過夜停車!'));
                $Lp_list2=Lp::all()->whereNotIn('status','已過夜停車!')->sortBy('status');
                $Lp_list=$Lp_list1->merge($Lp_list2);
                return $Lp_list;
                break;
            case 'ban':
                $Lp_list1=collect(Lp::all()->where('status','黑名單'));
                $Lp_list2=Lp::all()->whereNotIn('status','黑名單')->sortBy('status');
                $Lp_list=$Lp_list1->merge($Lp_list2);
                return $Lp_list;
                break;
            default:
